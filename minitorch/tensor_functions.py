...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return (
            grad_output
            * (1 + t1.f.neg_map(t1.f.sigmoid_map(t1)))
            * t1.f.sigmoid_map(t1)
        )
...
class ReLU(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        # TODO: Implement for Task 2.4.
        (t1,) = ctx.saved_values
        return grad_output * (t1.f.relu_map(t1) > 0)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        permuted_tensor_data = a._tensor.permute(*order_list)
        return a._new(permuted_tensor_data)
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        permuted_grad_data = grad_output._tensor.permute(*inverse_order)
        permuted_grad = grad_output._new(permuted_grad_data)
